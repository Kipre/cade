...
bool WriteCompoundToSTEPString(const TopoDS_Compound& compound, std::string& stepString) {
    // Create STEP writer
...
// Alternative approach using memory stream (if available in your OpenCascade build)
bool WriteCompoundToSTEPStringDirect(const TopoDS_Compound& compound, std::string& stepString) {
    STEPControl_Writer writer;
    ...
    // Note: This approach requires OpenCascade to be built with stream support
    // which may not be available in all builds
    // status = writer.Write(oss);
    
    // For now, use the file-based approach above
    return WriteCompoundToSTEPString(compound, stepString);
}
           ...
           const Standard_Real myThickness)
{
  ...
  if (WriteCompoundToSTEPString(aRes, stepContent)) {
