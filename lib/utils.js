...
import { eps, norm } from "../tools/2d.js";
...
export function locateOriginOnFlatPart(parent, flatPart, otherPart) {
  ...
  const point = proj2d(
    transformPoint3(place1.inverse().multiply(place2), zero3),
  );
  return point;
}
 ...
 * @param {BasePart} otherPart
 */
export function locateOriginsOnFlatPart(parent, flatPart, otherPart) {
  const place1 = parent.findChild(flatPart).placement;
  const places = parent.findChildren(otherPart);

  const result = [];
  for (const located of places) {
    const loc = proj2d(
      transformPoint3(place1.inverse().multiply(located.placement), zero3),
    );
    if (result.some((p) => norm(loc, p) < eps)) continue;
