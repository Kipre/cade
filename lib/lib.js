  ...
  *getPairings() {
  ...
  mirror(normal = [0, 0, 1]) {
    function mirrorChild(child, placement) {
      ...
      const symmetryPlaneNormal = transformPoint3(
        placement.inverse(),
        normal,
        true,
      );
      const revUp = transformPoint3(placement, z3, true);
      const secondAxis = transformPoint3(placement, cross(normal, revUp), true);

      const ZERO = transformPoint3(placement.inverse(), zero3);

      // maybe find a better way
      const symmetryAxis = symmetryPlaneNormal.findIndex(
        (x) => Math.abs(x) > eps,
      );
      const secondAxisIdx = [
        secondAxis.findIndex((x) => Math.abs(x) > eps),
      ].filter((x) => x !== -1);

      const others = [0, 1, 2].filter(
        (x) => x !== symmetryAxis && x !== secondAxisIdx[0],
      );
      const sym = [symmetryAxis, ...secondAxisIdx, ...others].map(
        (i) => child.symmetries[i],
      );

      if (!Object.is(NaN, child.symmetries[symmetryAxis])) {
        const symmetryCenter = plus3(mult3(ZERO, -1), child.symmetries);
        const translation = -2 * symmetryCenter[symmetryAxis];

        const v = [0, 0, 0];
        v[symmetryAxis] = translation;

        return new DOMMatrix().multiply(placement).translate(...v);
      }
      ...
      if (!Object.is(NaN, sym[2])) {
        const axis = cross(revUp, normalize3(normal));
        return new DOMMatrix()
          .translate(zero[0], zero[1])
          .translate(...mult3(revUp, sym[2]))
