...
import { dot, eps, plus } from "../tools/2d.js";
import { collinear3, cross, dot3, minus3, mult3, norm3, normalize3, plus3, project3 } from "../tools/3d.js";
import { a2m, atm3, computeAngleBetweenVectors, intersectPlanes, transformPoint3 } from "../tools/transform.js";
...
export class BasePart {
  ...
  /**
     * @param {DOMMatrix[]} symmetries
     */
  addSymmetries(...symmetries) {
    this.newSymmetries.push(...symmetries);
  }

  getSymmetries() {
    return this.newSymmetries;
  }

  /**
   * @param {DOMMatrix} normal
   * @returns {LocatedPart}
   */
  enantiometer(normal) {
    const [symmetry] = this.getSymmetries();
    if (!symmetry) throw new Error(`cannot mirror part ${this.name}`);

    if (collinear3(atm3(normal, z3, true), atm3(symmetry, z3, true))) {
      const planeToPlane = atm3(normal.inverse().multiply(symmetry), zero3)[2];
      return {
        placement: normal.translate(0, 0, -2 * planeToPlane).multiply(normal.inverse()),
        child: this,
      }
    }

    const intersectionAxis = intersectPlanes(normal, symmetry)
    const n1 = atm3(normal, z3, true);
    const n2 = atm3(symmetry, z3, true);
    const angle = computeAngleBetweenVectors(n1, n2);

    const placement = intersectionAxis.rotate(0, 0, -2 * 180 * angle / Math.PI)
      .multiply(intersectionAxis.inverse());

    return { placement, child: this }
  }

  /**
   * @returns {Generator<LocatedPart & {parent: Assembly}>}
   */
  *getPairings() { }
...
export class Assembly extends BasePart {
  ...
  *getPairings() {
  ...
  getSymmetries() {
    const [first, ...otherChildren] = this.children;
    let result = first.child.getSymmetries()
      .map(s => (first.placement ?? a2m()).multiply(s));

    for (const { child, placement: m } of otherChildren) {
      const placement = m ?? a2m();

      const syms = child.getSymmetries();
      const filtered = [];

      for (const sym of result) {
        const one = atm3(sym, z3, true);
        const zero = project3(atm3(sym, zero3), one);

        for (const childSym of syms) {
          const located = placement.multiply(childSym);

          const childOne = atm3(located, z3, true);
          const childZero = project3(atm3(located, zero3), one);

          if (norm3(childZero, zero) < eps && collinear3(one, childOne)) {
            filtered.push(sym);
          }
        }
      }
  ...
  enantiometer(normal) {
    ...
    if (symmetry) {
      return super.enantiometer(normal);
    }

    const result = new Assembly(`mirrored ${this.name}`);

    for (const { child, placement: m } of this.children) {
      const initialLocation = m ?? a2m();
      const mirror = initialLocation.inverse().multiply(normal)

      const { child: c, placement } = child.enantiometer(mirror)
      result.addChild(c, initialLocation.multiply(placement));
    }

    return { child: result, placement: a2m() }
  }
  ...
  *findChildren(part) {
  ...
  async project(file) {
    ...
    return await fetch(`/occ/project?${params}`, {
      method: "POST",
      body,
    });
  }
