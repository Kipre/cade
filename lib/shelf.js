...
import { areOnSameLine, dot, eps, intersectLines, minus, norm, offsetPolyline, placeAlong, pointToLine, polygonCenter, signedArea } from '../tools/2d.js';
...
export class ShelfMaker {
  ...
  addFlatPart(locatedPart) {
    ...
    this.locatedFlatParts.push({ locatedPart, type: "normal" });
    return this;
  }
  ...
  addSingleSideOfPart(locatedPart, otherSide = false) {
    this.locatedFlatParts.push({ locatedPart, type: otherSide ? "leftSide" : "rightSide" });
    return this;
  }
  ...
  make(debug = false) {
    ...
    for (const { locatedPart, type } of this.locatedFlatParts) {
      ...
      if (line.length === 2) {
        ...
        if (type === "leftSide") {
          geometries.push({ pts: line.map((p) => parttp([...p.point, 0])), type: "edge", pairing: "left", zee });
        } else if (type === "rightSide") {
          geometries.push({ pts: line.map((p) => parttp([...p.point, part.thickness])), type: "edge", pairing: "right", zee });
      ...
      const centerline2 = projectCenterLine(planeToPart, this.opts.woodThickness, 100);
