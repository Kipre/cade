...
import { cut, extrusion } from "./operations.js";
import { Part } from "./part.js";
import { locateOriginsOnFlatPart } from "./utils.js";
 ...
 */
export function fastenSubpartToPart(
  parent,
  subpart,
  part,
  holeIterator,
  ...
  for (const located of parent.findChildren(subpart)) {
    ...
    for (const { hole, depth, transform: holeTransform } of holeIterator(
      ...
      const requiredClampingLength = depth + thickness;
      ...
      const holeOnPart = holeToSubToPart.translate(...location);

      const drill = extrusion(
        holeOnPart.translate(0, 0, -1 * thickness),
        thickness * 3,
      ...
      if (!skipNut) {
        ...
        result.push(locatedBottom.placement);
      }
    }
  }
  part.shape = cut(part.shape, ...drills);
  return result;
}
...
export function clearBoltOnFlatPart3(
  ...
  if (idx == null) {
    debugGeometry(flatPart.outside, points);
    console.log(points);
    console.log(
      flatPart.outside.findSegmentsOnLine(...points.slice(0, 2), {
        exact: true,
      }),
    );
  }
