...
import { cross, minus3, norm3, proj2d } from "../tools/3d.js";
...
export function makeShelfOnPlane(
  ...
  for (const { child: part, placement } of locatedFlatParts) {
    ...
    const planeToPart = (placement ?? a2m())
      .inverse()
...
export function joinParts(parent, toTab, toSlot, ...layouts) {
  ...
  if (intersections.length % 2 !== 0) throw new Error("expected an even number of intersections");
  ...
  for (let i = 0; i < intersections.length; i += 2) {
    ...
    const centerline = line.map(p => transformPoint3(tabToSlot.inverse(), [...p.point, toSlot.thickness / 2]));
  ...
  if (segments.length !== layouts.length) {
    const err = new Error("mismatch between nb of layouts and number of segments");
    console.error(err);
  ...
  for (let i = segments.length - 1; i >= 0; i--) {
    ...
    for (const slot of layout.toReversed()) {
      ...
      const { path, booleanDifference, fastener, slotPlacement } = slot.materializeOuter(
        toTab,
        segmentIdx,
        place,
      );
...
export function spindleClearedLineTo(
  ...
  function getIntermediatePoint(lp, p, np, radius) {
    const angle =
      normalizeAngle(Math.atan2(...minus(np, p).toReversed()) -
 ...
 * @param {number} radius
 */
export function spindleCleared2LineTo(
  path,
  np,
  radius,
) {
  ...
  const angle = normalizeAngle(Math.atan2(...minus(np, p).toReversed()) -
