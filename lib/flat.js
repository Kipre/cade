...
import { nx3, ny3, x3, y3, z3, zero3 } from "./defaults.js";
...
export class FlatPart extends Part {
  ...
  mirror(p1, p2) {
    if (p1 && p2) {
      ...
      this.outside = this.outside.moveClosingSegment(segment);
    }
    ...
    for (let i = 0; i < length; i++) {
      this.insides.push(this.insides[i].transform(mat).invert());
    }
  }
 ...
 * @returns {[types.Point, types.Point]}
 */
export function projectPlane(planeDef, transform) {
  const [start, end] = [zero3, z3].map((p) =>
    proj2d(transformPoint3(transform.multiply(planeDef), p)),
  );
...
export function joinParts(
  ...
  for (let i = segments.length - 1; i >= 0; i--) {
    ...
    const layout = maybeLayouts[i] ?? defaultSlotLayout(segmentLength);

    // reversing for conserving the segment idx
    for (const slot of layout.toReversed()) {
      let place = slot.x;
      if (maybeLayouts[i]) place = slot.x * segmentLength;
      const { path, location, vector } = slot.materialize(
        toTab,
        segmentIdx,
        place,
      );
      const center = tts([...location, toTab.thickness / 2]);
      const dir = minus(tts([...vector, toTab.thickness / 2]), center);
      let rotation = computeVectorAngle(dir);
      toSlot.addInsides(path.rotate(rotation).translate(center));
    }
  }
}
