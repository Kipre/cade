...
import { cross, minus3, plus3, proj2d } from "../tools/3d.js";
...
export class FlatPart extends Part {
  ...
  constructor(name, thickness, outside, insides = []) {
    ...
    this.insides = this.shape.retreive().insides;
    this.symmetries = /** @type {types.Point3} */ ([
      NaN,
      NaN,
      this.thickness / 2,
    ]);
    this.addSymmetries(a2m([0, 0, thickness / 2], z3));
  }
  ...
  display() {
    ...
    const [, , width, height] = bbox.toViewBoxArray();
...
export function joinParts(parent, edgePart, otherPart, ...layouts) {
  ...
  for (let i = 0; i < intersections.length; i += 2) {
    ...
    const centerline = line.map((p) =>
      atm3(edgeToOther.inverse(), [...p.point, otherPart.thickness / 2]),
    );
    ...
    for (const side of [side1, side2]) {
      for (const idx of edgePart.outside.findSegmentsOnLine(...side, {
        exact: true,
      })) {
        ...
        segments.push({ idx, start, end, startPoint: p1, endPoint: p2 });
      }
    }
  }
  ...
  segments.sort(keyToComparison2d((v) => v.startPoint));

  for (let i = segments.length - 1; i >= 0; i--) {
    const { idx, start, end, startPoint, endPoint } = segments[i];

    const width = end - start;
    const segmentLength = norm(endPoint, startPoint);
    ...
    for (const slot of layout.toReversed()) {
      ...
      if (slot instanceof BaseSlot) {
        materialization = slot.materializeOuter(
          edgePart,
          startPoint,
          endPoint,
          place,
        ...
        materialization = slot.materializeOuter(edgePart, idx, place);
      }
 ...
 * @param {(BaseSlot[] | ((length: number) => BaseSlot[]))[]} layouts
 */
export function makeRoomForATenon(parent, edgePart, otherPart, spindleSize) {
  const otherInstances = [...parent.findChildren(otherPart)];
  const edgeInstances = [...parent.findChildren(edgePart)];
  ...
  const side1 = offsetPolyline(centerline, otherPart.thickness / 2);
  const side2 = offsetPolyline(centerline, -otherPart.thickness / 2);
  const segments1 = edgePart.outside.findSegmentsOnLine(...side1);
  const segments2 = edgePart.outside.findSegmentsOnLine(...side2);

  if (
    segments1.length + segments2.length !== 2 ||
    segments1.length * segments2.length !== 0
  )
    throw new Error("cannot make slot in these conditions");

  const [s1, s2] = [...segments2, ...segments1];

  const [, p1, , p2] = edgePart.outside.getSegmentAt(s1);
  const [, p3, , p4] = edgePart.outside.getSegmentAt(s2);

  const [, start, end] = [p1, p2, p3, p4].toSorted(
    keyToComparison((v) => norm(v, centerline[0])),
  );
  const slotLine = [start, end].map((v) =>
    proj2d(atm3(edgeToOther, [...v, edgePart.thickness / 2])),
  );

  const [v1, v2] = offsetPolyline(slotLine, edgePart.thickness / 2);
  const [v4, v3] = offsetPolyline(slotLine, -edgePart.thickness / 2);

  const slot = new Path();
  const beginning = placeAlong(v1, v2, { fraction: 0.5 });
  slot.moveTo(beginning);
  slot.lineTo(v2);
  spindleClearedLineTo(slot, v3, spindleSize / 2, true);
  spindleClearedLineTo(slot, v4, spindleSize / 2, false);
  spindleClearedLineTo(slot, v1, spindleSize / 2, true);
  spindleClearedLineTo(slot, beginning, spindleSize / 2, false);
  slot.close();
  slot.simplify();

  // otherPart.addInsides(slot);
  otherPart.assignOutsidePath(otherPart.outside.booleanDifference(slot));
}
...
export function getFaceOnLocatedFlatPart(locatedPart, keyFunc) {
  const { placement, child: flatPart } = locatedPart;
  // if (norm(cross(atm3(placement, z3), z3)) < eps) {
  //   if (dot3(atm3(placement, z3), z3) > eps)
  //     return placement.translate(0, 0, flatPart.thickness);
  //   return placement.multiply(a2m(zero3, nz3));
  // }
  ...
  for (const [, lp, type, p] of flatPart.outside.iterateOverSegments()) {
    ...
    const mp = placeAlong(lp, p, { fraction: 0.5 });
    const middle = atm3(placement, [...mp, 0]);

    candidates.push({ middle, lp, p });
  }

  candidates.sort(keyToComparison((x) => keyFunc(x.middle)));
  const { lp, p } = candidates.at(-1);

  const zee = rotatePoint(lp, p, Math.PI / 2);
  return placement.multiply(a2m([...lp, 0], [...minus(zee, lp), 0]));
}
