...
export function joinParts(parent, toTab, toSlot, ...layouts) {
  ...
  for (let i = 0; i < intersections.length; i += 2) {
    ...
    for (const side of [side1, side2]) {
      for (const idx of toTab.outside.findSegmentsOnLine(...side, true)) {
        // normalize coordinates to find overlapping segments
        const [, p1, , p2] = toTab.outside.getSegmentAt(idx);
        const [s1, s2] = side;
        const m = computeTransformFromPoints(
  ...
  for (let i = segments.length - 1; i >= 0; i--) {
    ...
    const segmentLength = toTab.outside.getLengthInfo().info[idx - 1].length;
    ...
    for (const slot of layout.toReversed()) {
      ...
      const { path, booleanDifference, fastener, slotPlacement } =
        slot.materializeOuter(toTab, idx, place);
