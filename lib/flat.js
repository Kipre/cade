 ...
 * @param {types.Point3} normalVector
 * @param {DOMMatrix} transform
 * @returns {[types.Point, types.Point]}
 */
export function projectPlane(point, normalVector, transform) {
  const [start, end] = [point, normalVector].map((p) =>
    proj2d(transformPoint3(transform, p)),
  );
...
export function joinParts(
  ...
  for (let i = segments.length - 1; i >= 0; i--) {
    ...
    for (const slot of layout.toReversed()) {
      ...
      const { path, location } = slot.materialize(toTab, segmentIdx, place);
