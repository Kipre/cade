...
export function makeJoinFromEdgePoints(
  origin,
  xEnd,
  yEnd,
  woodThickness,
  ...
  const child = new FlatPart(
    `gantry join at ${origin}`,
    woodThickness,
    joinPath,
  );
 ...
 */
export function trimFlatPartWithAnother(parent, partToTrim, otherPart) {
  ...
  const path = partToTrim.outside;
  const bbox = new BBox();
  for (const p of path.getPointsWithHalfArcs()) {
    bbox.include(p);
  }

  const size = bbox.size();
  const toRemove = Path.fromPolyline([
    proj2d(transformPoint3(plane, [-2 * size, 0, 0])),
    proj2d(transformPoint3(plane, [2 * size, 0, 0])),
    proj2d(transformPoint3(plane, [2 * size, 0, 2 * size])),
    proj2d(transformPoint3(plane, [-2 * size, 0, 2 * size])),
  ]);

  partToTrim.assignOutsidePath(path.booleanDifference(toRemove));
}
