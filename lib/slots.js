...
export class TroughAngleSupport extends BaseSlot {
   ...
   * @param {number} x
   */
  constructor(locatedEdgePart, partToSupport, otherSide = false, spindleDiameter = 6) {
    super(0);
  ...
  materialize(part, segmentIdx, place) {
    ...
    const ordered = norm(pts[0], bbox.center()) < norm(pts[1], bbox.center()) ?
      pts : pts.toReversed();

    const [, s1, , s2] = part.outside.getSegmentAt(segmentIdx);

    const offset = offsetPolyline(ordered, (this.otherSide ? -1 : 1) * third.thickness / 2);
    const off = 20;
