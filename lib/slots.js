...
export class TenonMortise extends BaseSlot {
  ...
  materialize(part, segmentIdx, place) {
    ...
    const point = placeAlong(start, end, { fromStart: place ?? this.x });
    ...
    path.insert(segmentIdx, tenon.rotate(rotation).translate(point));

    return { path: mortise, location: point, vector };
  }
}
...
export class CylinderNutFastener extends BaseSlot {
  ...
  materialize(part, segmentIdx, place) {
    ...
    const center = rotatePoint(
      ...
      -Math.PI / 2,
      // path.rotatesClockwise() ? -Math.PI / 2 : Math.PI / 2,
    );

    part.addInsides(this.nutHole.translate(center));
    return { path: this.boltHole, location, vector };
  }
}
